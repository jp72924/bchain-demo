...
@dataclass
class COutPoint:
    hash: bytes  # 32-byte tx hash
    n: int       # Output index
    
    def __post_init__(self):
        if len(self.hash) != 32:
            raise ValueError("Invalid COutPoint hash length")
...
class CTransaction:
    def __init__(self, vin: List[CTxIn], vout: List[CTxOut], nLockTime: int = 0):
        self.vin = vin
        self.vout = vout
        self.nLockTime = nLockTime

    def get_hash(self) -> bytes:
        """Returns double SHA256 hash of serialized transaction"""
        return hash256(self.serialize())

    def serialize(self) -> bytes:
        stream = io.BytesIO()
        # Version (always 1 in v0.1)
...
if __name__ == "__main__":
    ...
    # push_pubkeys = (
    #     bytes([OP_2]) +
    #     bytes([OP_PUSHDATA1, 0x21]) + pubkey1 +
    #     bytes([OP_PUSHDATA1, 0x21]) + pubkey2 +
    #     bytes([OP_PUSHDATA1, 0x21]) + pubkey3 +
